/**
 * 肉鸽升级系统 - Roguelike Upgrade System
 * 版本：v3.1.0
 * 功能：经验系统、升级弹窗、技能库、3 选 1
 */
...
var RogueSystem = {
    ...
    init: function() {
        ...
        console.log('⚔️ 肉鸽系统初始化完成 v3.1.0');
    },
    ...
    setupControls: function() {
        ...
        this.canvas.addEventListener('mousemove', function(e) {
            ...
            self.player.angle = Math.atan2(e.clientY - self.player.y, e.clientX - self.player.x);
        });
        this.canvas.addEventListener('touchmove', function(e) {
            if (!self.running || self.paused) return;
            e.preventDefault();
            var t = e.touches[0];
            self.player.angle = Math.atan2(t.clientY - self.player.y, t.clientX - self.player.x);
        }, {passive: false});
        this.canvas.addEventListener('click', function() { if (self.running && !self.paused) self.shoot(); });
    },
    ...
    showLevelUpModal: function() {
        ...
        options.forEach(function(skill) {
            ...
            card.onclick = function() { RogueSystem.selectSkill(skill); };
    ...
    getRandomSkills: function(count) {
        var shuffled = this.skills.sort(function() { return 0.5 - Math.random(); });
    ...
    update: function() {
        ...
        if (this.player.shootTimer === 0 && Math.random() < 0.05) this.shoot();
        ...
        for (var i = this.enemies.length-1; i >= 0; i--) {
            ...
            for (var j = this.bullets.length-1; j >= 0; j--) {
                ...
                if (dist < e.width/2 + b.radius) {
                    ...
                    if (e.hp <= 0) {
                        this.enemies.splice(i, 1); this.score++; this.addExp(10); // 击杀获得 10 经验
    ...
    render: function() {
        ...
        this.ctx.fillStyle = this.player.color; this.ctx.beginPath(); this.ctx.moveTo(20,0); this.ctx.lineTo(-15,-15); this.ctx.lineTo(-15,15); this.ctx.closePath(); this.ctx.fill(); this.ctx.restore();
    ...
    restart: function() { document.getElementById('pause-menu').style.display = 'none'; document.getElementById('gameover-menu').style.display = 'none'; this.start(); },
